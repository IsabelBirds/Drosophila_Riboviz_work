...
test_3 <- unlist(all_threeUTR)
test_3$type <- "3UTR"
test_5 <- unlist(all_fiveUTR)
test_5$type <- "5UTR"
test_cds <- unlist(cds)
test_cds$type <- "CDS"

#Combine all granges - pc combines element wise
#all_sequences <- pc(all_fiveUTR,cds,all_threeUTR)
all_sequences <- c(test_5,test_cds,test_3)
export.gff(all_sequences,"tmp_all.gff3")


#Get sequences from fasta
#test <- unlist(all_sequences)
#test$type <- names(test)
#export.gff3(test,"tmp_cds.gff" )

#genome_test <- read.fasta(genome_path) 
#Split - splices together cds
test_seq <- bedtoolsr::bt.getfasta(fi=genome_path,bed = "tmp_all.gff3",nameplus =TRUE, split = FALSE)
head(test_seq)
```
